...
| full ROS graph validation | ROS graph 上の ASR-ready stream、VAD close、backend invocation、result publish。 | `fa_in -> processing -> fa_vad -> fa_asr` 相当の経路で non-empty final result が出ること。 | worker health や fixture smoke の代替ではない。 | identity / timeline / VAD / backend / publish のどこかが未確認なら未検証。 | `Verification Requirements` 5。 |
...
9. worker は restore 済み model に `transcribe([wav_path], batch_size=1, return_hypotheses=False, verbose=False, ...)` を呼ぶ。runtime signature が受け付ける場合だけ `language` / `num_workers` 系 keyword を渡す。
...
| file-source full ROS graph validation | 未検証 / PO 検証中。 |
