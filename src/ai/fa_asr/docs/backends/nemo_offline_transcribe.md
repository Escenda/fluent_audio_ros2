...
2026-05-22 時点で PO による real worker health、raw `FLOAT32LE` fixture transcription、opt-in file-source full ROS graph smoke は通過済みです。
file-source full graph は local Parakeet `.nemo` + `nemo_offline_transcribe` を使う stated env 条件で non-empty final result まで到達しました。
ただし、accuracy 評価、`TranscribeAudio` service integration、NIM / Riva / gRPC backend readiness、generic live microphone ASR は未検証です。
したがって、この backend を accuracy や汎用 live ASR まで含む本番相当経路で検証済みとは扱いません。
...
| local 実測 | `fluent-audio-runtime`、Torch `2.11.0+cu130`、CUDA available、`EncDecRNNTBPEModel` restore、offline fixture output | 対象 local `.nemo` を NeMo offline API で restore/transcribe できること。 | この direct API evidence 単体では、streaming backend 成功、accuracy、file-source full ROS graph 成功。 |
...
- generic real microphone live ASR。
...
accuracy は証明しません。
この direct API evidence 単体では、`nemo_rnnt_streaming` backend の `health_ok`、streaming `start/audio/finish`、partial/final transcript、file-source full ROS graph 成功も証明しません。
file-source full ROS graph success は、別 evidence である opt-in smoke の結果として扱います。
