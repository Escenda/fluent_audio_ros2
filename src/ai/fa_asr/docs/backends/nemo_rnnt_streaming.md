...
`nemo_rnnt_streaming` is a legacy experimental cache-aware streaming backend slot.
It is not the current standard ASR backend, not a default path, not a validation substitute, and not a fallback for `parakeet_multilingual_buffered`.
The current standard multilingual Parakeet path is `parakeet_multilingual_buffered`, which uses rolling-buffer full-context re-decode.
...
raw worker protocol としては、`start` command が prior `health` なしで届いた場合にも、worker は `.nemo` restore / capability validation を実行してから stream を開始します。
`health` 後に `start` command が届いた場合、worker は start message の config を health 済み worker config と比較し、不一致なら `start stream contract does not match worker health contract` として拒否します。
これは transport protocol 上の許容経路と consistency check であり、system としての health-first 完了条件を弱めるものではありません。
...
## Backend-Adjacent Concept Boundaries
...
NIM の streaming inference は audio arrival に応じて partial transcript を返す serving mode です。

Parakeet RNNT Multilingual の NIM deployment では `CONTAINER_ID=parakeet-1-1b-rnnt-multilingual` を使い、`NIM_TAGS_SELECTOR` で profile を選びます。
`mode=ofl`、`mode=str`、`mode=str-thr`、`mode=all`、`diarizer=sortformer`、`vad=silero`、`type=default`、`type=prompt`、`type=indic` は NIM serving profile の概念です。

FluentAudio の現行 `nemo_rnnt_streaming` backend は NIM container を使いません。
NIM support matrix 上の streaming / offline / `ja-JP` support は model family と serving profile の根拠ですが、local `.nemo` worker の `health`、`start`、`audio`、non-empty `final` 成功を証明しません。

### Riva

Riva は NVIDIA の speech serving / deployment stack です。
Riva server、model repository、Triton、serving pipeline、profile selection、pre/post-processing、offline / streaming recognition と結びつきます。

この backend は Riva server を起動せず、Riva model repository を runtime として使いません。
Riva model card は artifact selection の参考にできますが、Riva serving capability を FluentAudio local worker の capability として扱いません。
`model format=riva` や `Riva 2.19.0 or higher` という metadata は Riva serving stack 向けの情報であり、local NeMo 2.7.3 worker の streaming 成立とは別に検証します。
...
現行 `nemo_rnnt_streaming` backend は gRPC endpoint に接続しません。
`fa_asr` と `nemo_rnnt_streaming_worker` は stdin / stdout の JSON Lines protocol で通信します。
stdout は JSONL response 専用であり、NeMo warning、traceback、telemetry は stderr / trace 側へ分離します。

NIM / Riva gRPC serving を使う場合は、この local `.nemo` backend に混ぜず、別 backend slot として設計します。
その場合の成功条件は gRPC readiness、request / response schema、network failure、server-side profile selection を含む別 contract になります。

### NGC Artifact / `download-version`

NGC artifact は model file と version metadata の配布単位です。
現時点の検証対象 artifact は次です。

```text
nvidia/riva/parakeet-rnnt-riva-1-1b-unified-ml-cs-universal:trainable_v1.0
```

NGC CLI の `registry model download-version` は、指定 version の contents を local disk に取得する操作です。
`--dest` は取得先 directory、`--file` は取得 file の wildcard、`--exclude` は除外 wildcard、`--format_type` は CLI output format です。

`download-version` は次を行いません。

- Riva artifact を local NeMo streaming 用に変換する。
- `.nemo` の model config を変更する。
- finite attention context を追加する。
- `ASRModel.restore_from(...)` を実行する。
- worker `health` を実行する。
- NIM / Triton / Riva runtime を再現する。

version を省略すると NGC 側 latest を対象にするため、FluentAudio の backend 検証では version を省略しません。
download 成功は local disk 上に artifact contents が置かれたことだけを意味し、backend readiness ではありません。

### Local `.nemo`

local `.nemo` は `nemo_rnnt_streaming_worker` が `ASRModel.restore_from(...)` で直接 restore する checkpoint です。
model checkpoint は git 管理せず、host または prepare container で取得し、runtime container には volume として渡します。
runtime node 起動時に NGC CLI download を行う設計にはしません。

local file の size / SHA256 / metadata は integrity と selection の証跡です。
しかし file が存在し、restore でき、RNNT/BPE model として見えることは、streaming ASR 成功と同義ではありません。
local `.nemo` backend としては、sample rate、RNNT / Transducer capability、cache-aware streaming API、finite attention context、streaming params、worker protocol、speech fixture、full ROS graph を順に検証します。

### Cache-Aware Streaming

cache-aware streaming は、model に raw audio を渡して text を受け取るだけの wrapper ではありません。
NIM / Riva serving stack を使わないため、worker が model restore、capability validation、streaming setup、cache state、decoder state、error reporting を所有します。

worker は `preprocessor.window_stride` と `encoder.subsampling_factor` から raw sample chunk と encoder token chunk の関係を計算します。
`backend.chunk_size_samples` は raw audio sample 数の contract であり、NeMo encoder chunk size ではありません。
stream start ごとに `CacheAwareStreamingAudioBuffer`、encoder cache、previous hypothesis、previous decoder output を初期化し、session を跨いで共有しません。

`audio_accepted` は worker が audio chunk を active stream state に受理したことだけを意味します。
partial / final transcript、non-empty text、`finished` は別 event です。
empty final は transport completion として publish され得ますが、speech window の ASR 成功とは扱いません。

### Finite Attention Context

現行 worker policy では、local streaming backend として成立するには finite attention context が必要です。
restore 直後の model が full context `[-1, -1]` の場合、worker は `encoder.att_context_size_all` などから supported contexts を取得します。
現行固定 policy の requested finite context は `[70, 1]` です。

`[70, 1]` が supported contexts に含まれる場合だけ、worker は `set_default_att_context_size([70, 1])` を呼びます。
supported contexts が `[[-1, -1]]` だけの場合、または `[70, 1]` が含まれない場合、worker は fail closed します。

NeMo の `set_default_att_context_size(...)` は unsupported look-ahead で warning を出しても、その後 `att_context_size` をセットし、`setup_streaming_params()` を呼び得ます。
そのため FluentAudio worker は NeMo warning を成功扱いせず、呼び出し前の supported context 検査を必須にします。

### Full-Context / Offline / Simulated Streaming

full-context / offline ASR は、complete audio を処理して complete transcript を返す policy です。
NeMo docs には full-context model を streaming simulation に使う道がありますが、accuracy / latency tradeoff を持ち、limited-context streaming model とは別物です。

現行 `nemo_rnnt_streaming` backend は streaming 専用です。
`NemoRnntStreamingAsrBackend.transcribe()` は常に失敗し、`TranscribeAudio` service の non-streaming backend としては使いません。
full-context model を simulated streaming として使う場合は、現行 backend の finite context requirement を弱めるのではなく、別 backend policy として設計します。

### Language Profile

NIM support matrix の language profile、local `.nemo` の vocabulary / metadata、FluentAudio config の `backend.language` は同一 contract ではありません。
NIM の `type=default` は automatic language detection と language code output、`type=prompt` は client language code を要求する profile、`type=indic` は Indic languages 向け profile です。
`ja-JP` は NIM support matrix では default / prompt に含まれ、indic には含まれません。

local worker は model config が `languages` または `language` metadata を公開する場合だけ、`backend.language` と exact match で照合します。
model config が language metadata を公開しない場合、worker は language だけを理由に拒否しません。
その場合でも vocabulary に `<ja-JP>` があることは「日本語を扱う前提と矛盾しない」根拠に留まり、日本語 speech fixture の transcription 成功を代替しません。

過去の `nemo_rnnt_streaming` 実験では固定 language を profile に置く案がありましたが、
現行標準 ASR path では固定 language を標準設定にしません。
multilingual Parakeet の標準 path は `parakeet_multilingual_buffered` で、
`backend.language=""` と `backend.language_policy=auto_detect` を要求します。

### Worker Health / Trace

worker `health` は startup readiness の gate です。
`.nemo` restore、model class extraction、sample rate、RNNT / Transducer capability、cache-aware streaming API、language metadata、finite attention context、streaming params setup を audio arrival 前に検証します。
失敗する backend を runtime audio 到着まで遅延させません。
...
それらは worker 内で `health_ok` 返却前に検査 / 設定され、opt-in trace、stderr、worker internal validation の証跡として扱います。

現時点の Parakeet `.nemo` は `health` で次の理由により fail closed しています。

```text
model encoder does not support a finite attention context; supported_contexts=[[-1, -1]]
```

したがって、`health_ok`、`start`、`audio_accepted`、non-empty partial / final transcript、full ROS graph 上の ASR 成功は完了扱いしません。
...
`nemo_rnnt_streaming` としての full ROS graph は未検証です。
別 backend の `nemo_offline_transcribe` では、commit `43fabab3` の `pcm_file_reader` media-clock timestamp fix 後に、commit `5e3ae5a3` の opt-in file-source full graph smoke が `1 passed in 130.86s` で通過しています。
これは local Parakeet `.nemo` + offline backend の graph evidence であり、streaming `health_ok`、`start`、`audio_accepted`、non-empty partial / final transcript を証明しません。
pre-43fabab3 に観測された ASR-ready rolling timeline gap は、旧 timestamp contract の履歴証跡として扱い、現行 file-source offline graph の未完了理由にはしません。
...
これは replay や fallback ではなく、有限入力の先頭 frame を ROS discovery 前に捨てないための source-read gating です。

この backend の完了判定は、NGC download、file presence、NIM / Riva support matrix、unit test 成功では行いません。
local `.nemo` に対する `health -> start -> audio -> finish`、speech fixture、そして full ROS graph 上の non-empty final result までを分けて確認します。

### Current State Matrix

| 状態 | 現在の状態 | 証明すること | 証明しないこと |
| --- | --- | --- | --- |
| NIM docs capability | 確認済み | NIM / Riva serving profile 上の Parakeet RNNT Multilingual capability。 | FluentAudio local `.nemo` worker の success。 |
| NGC downloaded | Parakeet artifact は取得済みとして扱うが、現行 prepare script とは別作業 | pinned version contents が local disk に存在すること。 | `.nemo` restore、finite attention context、worker health。 |
| local file integrity | size / SHA256 確認済み | local file が NGC metadata と整合すること。 | ASRModel restore 成功、ASR transcript 成功。 |
| `ASRModel.restore_from(...)` | 実 Parakeet `.nemo` で restore 到達済み | NeMo runtime が model file を読み、`EncDecRNNTBPEModel` として見えること。 | streaming backend 成立、non-empty transcript。 |
...
| full ROS graph | streaming backend としては未検証。別経路の offline file-source smoke は local Parakeet `.nemo` + `nemo_offline_transcribe` で通過済み。 | offline backend の graph binding、VAD control、ASR invocation、non-empty final result が stated env 条件で成立したこと。 | streaming `health_ok`、streaming non-empty partial / final result、NIM / Riva serving capability、generic live microphone ASR。 |
...
- `backend.chunk_ms`: `0`。現行実装では ms 指定を受け付けず、`backend.chunk_size_samples` を明示する。
...
`backend.chunk_ms` を 0 以外にして chunk size を暗黙換算する経路は現行実装にはありません。
...
- local `.nemo` を NeMo runtime が model object として restore できたこと。

この phase が証明しないこと:

- finite attention context、cache-aware streaming step、worker health、transcript success、full ROS graph success。
...
- local artifact と backend config から、以降の capability validation に必要な値を取り出せたこと。

この phase が証明しないこと:

- NIM/Riva serving profile の成功、streaming step 成功、speech transcript 成功。
...
- model class、sample rate、language metadata、partial support が backend config と矛盾しないこと。

この phase が証明しないこと:

- requested finite attention context が成立すること、`health_ok` が返ること、音声 chunk から text が返ること。
...
- requested finite context が supported contexts に含まれる場合だけ、local streaming policy を開始できる前提があること。

この phase が証明しないこと:

- transcript success、NIM/Riva serving success、full ROS graph success。
...
- selected finite attention context を前提に、encoder streaming parameters を作れたこと。

この phase が証明しないこと:

- stream session が始まったこと、audio chunk が受理されたこと、partial/final transcript が返ること。
...
`start` が prior `health` なしで届いた場合、worker は start message の config から restore / capability validation / streaming setup を行います。
`health` 後に `start` が届いた場合、worker は start message の config を health 済み worker config と比較し、不一致なら `start stream contract does not match worker health contract` として fail closed します。
backend wrapper は health と start に同じ resolved config を送ります。
health-first は system 側の完了条件であり、start-only transport path はその完了条件を弱めるものではありません。

この phase が証明すること:

- session-local cache と decoder state を初期化し、active stream を開始できたこと。

この phase が証明しないこと:

- audio chunk が valid であること、streaming step が進むこと、transcript が返ること。
...
- JSONL `audio` payload が active stream の raw float32le chunk として受け取れること。

この phase が証明しないこと:

- buffer が推論可能 chunk を返すこと、NeMo step が成功すること、transcript が返ること。
...
- validated audio を session-local buffer に追加し、推論可能な chunk を取り出せるか確認したこと。

この phase が証明しないこと:

- speech content を認識したこと、partial/final text が non-empty であること。
...
- conformer cache-aware streaming step が実行され、cache / hypothesis / decoder state を更新できたこと。

この phase が証明しないこと:

- transcript が正しいこと、言語 support 全般、ROS graph success。
...
- partial / drain / finish の protocol completion と commit semantics が成立したこと。

この phase が証明しないこと:

- non-empty transcript success。non-empty final または prior non-empty partial commit が無い限り、speech ASR 成功ではありません。
...
- failure phase と runtime state を後から追える証跡を残せること。

この phase が証明しないこと:

- worker health、transcript success、full ROS graph success。
...
pre-43fabab3 の file-source full graph 試行で観測された ASR timeline gap は、この contract により失敗として扱った履歴証跡です。
commit `43fabab3` 後の `pcm_file_reader` は actual PCM frame 数に基づく media-clock timestamp を使い、別 backend の `nemo_offline_transcribe` file-source smoke は通過済みです。
この事実は `nemo_rnnt_streaming` の `health_ok` や streaming transcript success を証明しません。
