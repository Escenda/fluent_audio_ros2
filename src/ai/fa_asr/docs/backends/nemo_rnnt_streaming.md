...
2026-05-22 時点の調査・実機検証では、次の事実が確認されています。
...
- `encoder.setup_streaming_params(...)`、finite attention context、`encoder.set_max_audio_length(...)` を worker 側で明示設定する実装は入っている。
- worker 単体では、無音 `FLOAT32LE` 16 kHz mono の `health -> start -> audio -> finish` が worker crash なしに完了し、silence として empty final を返す smoke test まで確認済み。
- unit test は `src/ai/fa_asr/test/unit/test_nemo_rnnt_streaming_worker.py` で `27 passed` を確認済み。

一方で、speech fixture と full ROS graph の代表検証はまだ完了していません。

- Wikimedia の短い日本語音声 fixture を file source から full ROS graph に流した検証では、`fa_vad` が `is_speech=false` のまま control window を開かず、ASR result は発生していません。この結果は VAD threshold / fixture 適合性の検証であり、ASR backend の transcript 成功証明ではありません。
- 同じ日本語音声を worker へ直接流した検証では、`health` と `start` は成功しましたが、audio push 中に response timeout となり、その後 pipe が閉じました。この経路は未解決です。

したがって、現状の backend は「model restore / health / minimal streaming smoke までは検証済み」ですが、「実 speech chunk を継続的に受理して non-empty partial / final transcript を返す streaming ASR backend」としては未検証です。
...
| --- | --- | --- |
| NIM | NVIDIA が配布する speech ASR serving container。model download、optimization、Triton/TensorRT を含む serving stack を container 内で持つ。 | 使わない。FluentAudio の `nemo_rnnt_streaming` は NIM server へ gRPC/WebSocket 接続しない。 |
| Riva | NVIDIA の speech serving SDK / model deployment stack。Riva model repository、Triton、pre/post-processing と結びつく。 | model artifact の入手元として扱う。Riva server が動くことは FluentAudio worker の成功証明ではない。 |
...
### NGC model card / version metadata

NGC CLI で確認した model metadata は次です。

```text
model: nvidia/riva/parakeet-rnnt-riva-1-1b-unified-ml-cs-universal
displayName: RIVA Parakeet-RNNT-XXL-1.1B ASR Multilingual Universal Tokenizer
latestVersionIdStr: trainable_v1.0
modelFormat: riva
precision: fp16
latestVersionSizeInBytes: 4011233560
```

`trainable_v1.0` version metadata は次です。

```text
status: UPLOAD_COMPLETE
totalFileCount: 1
totalSizeInBytes: 4011233560
Model Details:
  NeMo Version: 1.23.0
  Dataset Size: 94k hrs
  Vocabulary Size: 8.5k
```

model card 上の重要な事実は次です。

- Parakeet-RNNT-XXL-1.1B / FastConformer-RNNT 系の model。
- 25 languages を transcribe する model として記載されており、`ja-JP` を含む。
- input は audio / wav / mono channel required と記載されている。
- Runtime Engine は Riva `2.19.0` 以上と記載されている。
- model card 上の runtime は Triton / Riva を前提にしており、FluentAudio local worker の streaming 成功を保証するものではない。

ここで注意すべき点は、NGC 上の `modelFormat: riva` と、FluentAudio が local `.nemo` を直接 restore している事実を混同しないことです。
今回の `trainable_v1.0` は local volume 上では `.nemo` file として取得・配置されていますが、NGC model card は Riva deployment を主語にしています。
そのため、NGC metadata は artifact selection と model capability の根拠であり、`nemo_rnnt_streaming_worker` の実行契約は local NeMo source と worker 検証で別途証明します。

現時点の確認では、`ngc` CLI は host 側 `/home/user/repositories/daihen-physical-ai/ngc-cli/ngc` には存在しますが、`fluent-audio-runtime` container 内の PATH には存在しません。
したがって、model download / prepare は「runtime container 内で NGC CLI を実行する」前提にしてはいけません。
実装・運用上は、NGC CLI がある host または prepare container で artifact を取得し、取得済み `.nemo` を volume として FluentAudio runtime に渡す契約にします。
...
- `backend.max_partial_interval_ms`: partial interval contract。現行 worker は config validation / health response で保持するが、partial throttling はまだ実装していない。
...
worker level では、silence smoke test のように final text が空でも transport / streaming state の検証としては成功し得ます。
ただし、VAD / KWS / TD などの control input により「speech window」として開いた区間で final transcript が空の場合、それを音声認識成功として扱ってはいけません。
`fa_asr` 側は control window の理由、accepted sample count、partial 履歴、backend final を見て、empty final を明示的な event / result reason として記録する必要があります。
empty final を別 backend へ fallback したり、「ユーザーが何も言わなかった」と暗黙解釈したりしません。
...
| health response | effective capability | model class、sample rate、partial support、requested chunk / interval を返す。現行 worker は detailed `streaming_cfg` をまだ返さない。 | health success を audio success と混同しない。 |
...
過去の worker 実装では、`CacheAwareStreamingAudioBuffer` から得た chunk を `model.conformer_stream_step(...)` に渡す一方、step 直前に encoder の maximum audio / positional encoding length を cache 長込みで更新していませんでした。
...
現行 worker はこの契約を `CacheAwareConformerStreamingRunner._ensure_encoder_max_audio_length(...)` として実装しています。
この修正により、無音 audio の minimal streaming smoke では `audio_accepted` と `final` が返ることを確認済みです。
...
現行 worker は `backend.chunk_size_samples`、`backend.sample_rate_hz`、`model.cfg.preprocessor.window_stride`、`model.cfg.encoder.subsampling_factor` から encoder chunk size を導出します。

```text
encoder_token_samples = sample_rate_hz * window_stride * subsampling_factor
...
これは「audio JSONL で送る raw sample chunk」と「NeMo encoder が streaming step として扱う token/chunk」を明示的に橋渡しするための計算です。
`chunk_size_samples` をそのまま NeMo `chunk_size` に渡してはいけません。
window stride と subsampling factor を通さないと、raw audio sample と encoder token の単位が混ざり、cache / valid output / latency の意味が壊れます。

現行 worker は full-context `att_context_size=[-1, -1]` を検出した場合、`set_default_att_context_size([70, 1])` を呼んでから `setup_streaming_params(...)` を実行します。
この `[70, 1]` は現時点の実装上の固定 policy であり、将来は backend config から明示指定できるようにする余地があります。
ただし、固定 policy であっても silent fallback ではありません。
encoder が `set_default_att_context_size(...)` を提供しない、または finite context 設定に失敗する場合は fail closed します。

現行 worker は effective streaming config を health response にまだ詳細出力していません。
Product としては、`streaming_cfg.chunk_size`、`shift_size`、`last_channel_cache_size`、`valid_out_len`、`pre_encode_cache_size`、`drop_extra_pre_encoded` を trace / health detail に残す改善が必要です。
...
## Observed Validation And Remaining Failures

過去の full ROS graph 検証では、次の順序までは到達しました。
...
その後、worker 実装へ maximum audio length 更新と streaming params setup を入れ、無音の minimal streaming smoke は通過しています。

2026-05-22 の追加検証では、次も確認しています。

- `nemo_rnnt_streaming_worker` unit test: `27 passed`
- local runtime: NeMo `2.7.3` / PyTorch `2.11.0+cu130`
- worker direct silence smoke: `health_ok -> stream_started -> audio_accepted -> final(text="") -> finished`
- file source full graph with Japanese Wikimedia fixture: VAD が speech start を出さず、ASR window は開かない
- worker direct Japanese fixture: `health_ok` と `stream_started` 後、audio push 中に timeout / broken pipe

このため、過去の shape mismatch は「既知修正済みの failure」です。
しかし、speech fixture で non-empty transcript を返すこと、full ROS graph で `STATUS_FINAL` / non-empty text を得ることはまだ未検証です。
現在の主な未解決点は、長い/実 speech chunk を流したときの worker response latency、chunk drain cadence、partial emission、finish までの backpressure です。
...
このうち 1、3、4、5、6、7、8、9、10 の基本形は現行 worker に実装済みです。
ただし、speech fixture で timeout したため、次は実装済み step の性能・応答 contract を検証する必要があります。

- `audio` command 1 回あたりの最大処理時間を trace する。
- `CacheAwareStreamingAudioBuffer` がいつ chunk を返すかを trace する。
- `conformer_stream_step(...)` の step duration を trace する。
- `audio_accepted` を「推論 step 完了後」に返す現在の形でよいか、または「buffer 受理」と「inference step」を event として分けるか判断する。
- `timeout_sec` は backend の異常検知であり、遅い推論を正常に見せるために単に伸ばすものではない。
...
- NVIDIA Speech NIM ASR docs
  ...
  - https://docs.nvidia.com/nim/speech/latest/asr/deploy-asr-models/parakeet-rnnt.html
  ...
  - NIM では chunk-by-chunk streaming client が gRPC endpoint へ接続すること。
- NVIDIA Riva ASR docs
  ...
  - Riva は model repository / optimized serving pipeline を生成する stack であり、local NeMo checkpoint をそのまま Riva server と同一視してはいけないこと。
- NGC CLI model metadata
  - `ngc registry model info nvidia/riva/parakeet-rnnt-riva-1-1b-unified-ml-cs-universal --format_type json`
  - `ngc registry model info nvidia/riva/parakeet-rnnt-riva-1-1b-unified-ml-cs-universal:trainable_v1.0 --format_type json`
  - model card、version id、artifact size、file count、NeMo version、dataset size、vocabulary size。
