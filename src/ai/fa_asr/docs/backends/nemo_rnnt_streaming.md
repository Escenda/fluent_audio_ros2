...
- health と start の config が一致しない。
...
4. final が空の場合、speech window の ASR 成功として扱わない。

silence smoke では empty final が transport / state-machine の確認として成立する場合があります。
しかし VAD / KWS / TD / TurnContext によって speech window として開いた区間で empty final が返った場合、それを「ユーザーが何も言わなかった」と暗黙解釈してはいけません。
...
- NVIDIA ASR NIM Parakeet RNNT deploy docs
  - https://docs.nvidia.com/nim/speech/latest/asr/deploy-asr-models/parakeet-rnnt.html
  - `CONTAINER_ID=parakeet-1-1b-rnnt-multilingual`、NIM container deployment、streaming inference client の説明。
  - これは NIM serving の説明であり、local `.nemo` worker の成功証明ではないこと。
- NGC CLI registry docs
  - https://docs.ngc.nvidia.com/cli/cmd_registry.html
  - `ngc registry model info` / `download-version` の target format、`--dest`、`--file`、`--exclude`、`--format_type` の意味。
