...
| NGC artifact | Riva / NIM 用 model artifact を取得する配布単位。model card や version metadata を持つ。 | checkpoint の入手元。metadata は model 選定根拠だが、runtime contract は local worker で別途検証する。 |
