...
def test_cmake_removes_native_sherpa_link_mode() -> None:
    ...
    assert "sherpa-onnx-c-api" not in cmake_text
    assert "sherpa_onnx_kws_backend_unavailable.cpp" not in cmake_text


def test_cmake_builds_runtime_targets_with_external_worker_backend() -> None:
    cmake_text = (PACKAGE_ROOT / "CMakeLists.txt").read_text(encoding="utf-8")
...
def test_node_uses_backend_execution_provider_parameter() -> None:
    ...
    assert 'declare_parameter<std::string>("backend.command");' in node_text
    assert 'declare_parameter<std::vector<std::string>>("backend.args");' in node_text
    assert 'declare_parameter<std::vector<std::string>>("backend.health_args");' in node_text
    assert 'declare_parameter<double>("backend.timeout_sec");' in node_text
    assert 'declare_parameter<std::string>("backend.workspace_dir");' in node_text
    assert 'declare_parameter<bool>("backend.cleanup_audio_files");' in node_text
...
def test_backend_keeps_vad_gate_mandatory() -> None:
    ...
    process_text = backend_text.split("std::optional<KwsDetection> SherpaOnnxKwsBackend::process", 1)[1]
    gate_index = process_text.index("passesVadGate(vad_prob, config_.vad_threshold)")
    worker_index = process_text.index("runCommand(")
    assert gate_index < worker_index
...
def test_detection_score_is_owned_by_backend() -> None:
    ...
    assert "det.score = score;" in backend_path.read_text(encoding="utf-8")
...
def test_wav_tool_requires_explicit_provider() -> None:
    ...
    assert "--command <path>" in tool_text
    assert "args.command.empty()" in tool_text
    assert "cfg.command = args.command;" in tool_text
...
def test_wav_tool_initializes_every_backend_config_field_explicitly() -> None:
    ...
    assert "double timeout_sec{std::numeric_limits<double>::quiet_NaN()};" in tool_text
    ...
    assert 'arg == "--command"' in tool_text
    assert 'arg == "--workspace_dir"' in tool_text
    assert 'arg == "--timeout_sec"' in tool_text
    ...
    assert "timeout_sec must be finite and positive" in tool_text
    ...
    assert 'cfg.args = {' in tool_text
    assert 'cfg.health_args = {' in tool_text
...
def test_backend_boundary_fails_closed_for_invalid_runtime_state() -> None:
    ...
    assert "formatArgs(" in header_text
...
def test_backend_validates_model_paths_and_numeric_config_before_c_api() -> None:
    ...
    constructor_text = backend_text.split("SherpaOnnxKwsBackend::SherpaOnnxKwsBackend", 1)[1]
    assert constructor_text.index("validateConfig();") < constructor_text.index("runCommand(")
    ...
    assert "backend.health_args must not include {audio}" in backend_text
