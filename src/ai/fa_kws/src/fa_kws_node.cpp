...
public:
  ...
  : rclcpp::Node("fa_kws")
  {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "fa_kws initialized: audio_topic=%s expected_source_id=%s expected_stream_id=%s output_topic=%s target_sr=%d provider=%s",
      audio_topic_.c_str(),
...
private:
  ...
  void onDebugTimer()
  {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "fa_kws status: frames=%llu samples=%llu last_audio=%.2fs sr=%d",
  ...
  void onAudio(const AudioFrame::SharedPtr msg)
  {
    ...
    try {
      detection = kws_backend_->process(samples, target_sample_rate_, now);
