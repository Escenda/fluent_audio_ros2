...
- `config/profiles/file_ja_voice_frontend.yaml`: raw PCM file を `pcm_file_reader` で読み、`fa_in -> fa_sample_format -> fa_resample -> fa_dc_offset_removal -> fa_high_pass -> fa_vad -> fa_asr` を通す日本語 file-source frontend profile。file path と NeMo offline ASR worker / model path は environment で明示する。
