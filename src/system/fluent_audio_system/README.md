...
- `config/profiles/so101_kws_frontend.yaml`: `fa_in -> fa_sample_format -> fa_resample -> fa_vad / fa_kws` を明示した VAD/KWS frontend profile。VAD / KWS の worker command、model path / model files、provider、worker args、KWS health args、QoS は FluentAudio system config に閉じる。ASR / Turn Detector はこの profile では起動しない。
- `config/profiles/so101_voice_frontend.yaml`: `fa_in -> fa_sample_format -> fa_resample -> fa_vad / fa_kws / fa_asr / fa_turn_detector` を明示した voice frontend profile。TurnContext publisher は含まず、会話 orchestration は上位 app が担う。
