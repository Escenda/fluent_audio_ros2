...
@pytest.mark.parametrize("package_name", sorted(SOURCE_BOUND_STREAMING_PACKAGES))
def test_site_binding_launch_applies_source_id_to_audio_window_without_ai_override(
    package_name: str,
) -> None:
    overrides = build_site_binding_overrides(
        fa_in_enabled=True,
    ...
    assert node_launch_parameters(
        ...
        "/tmp/node.params.yaml",
        {"input.source_id": "hw:CARD=Mic,DEV=0"},
    ]
