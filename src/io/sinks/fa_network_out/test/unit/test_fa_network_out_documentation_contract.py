...
def test_fa_network_out_has_standard_design_documents() -> None:
    ...
    assert (
        PACKAGE_ROOT / "docs" / "backends" / "network_pcm_sender_adapter.md"
    ).is_file()


def test_fa_network_out_is_not_declared_as_ros_package() -> None:
    assert not (PACKAGE_ROOT / "package.xml").exists()
    assert not (PACKAGE_ROOT / "CMakeLists.txt").exists()


def test_fa_network_out_documents_current_backend_boundary() -> None:
    ...
    assert "package: fa_network_out" in readme
    assert "fa_out" in readme
    assert "backend.name: network_pcm_sender" in spec
    assert "current implementation lives in `fa_out`" in backend


def test_fa_network_out_documents_streaming_boundary() -> None:
    combined = "\n".join(
