...
TEST_F(RclcppContractTest, PublishesOverlappedWindowsAndAdvancesRetainedHeader)
{
  ...
  publisher->publish(frameAt(1000000000LL, 1U, {0.1F, 0.2F, 0.3F, 0.4F}));
  ...
  publisher->publish(frameAt(1004000000LL, 2U, {0.5F, 0.6F}));
  ASSERT_TRUE(spinUntil(executor, [&received]() {
    return received.size() == 2u;
  }));
  EXPECT_EQ(received[1].epoch, 1U);
  EXPECT_EQ(stampToNanoseconds(received[1].header.stamp), 1002000000LL);
  expectSamplesNear(readSamples(received[1].data), {0.3F, 0.4F, 0.5F, 0.6F});
}

TEST_F(RclcppContractTest, InvalidFrameDoesNotDiscardValidPartialBuffer)
{
  auto node = std::make_shared<fa_chunk_overlap::FaChunkOverlapNode>(
    optionsWith(validParameters()));
  ...
  publisher->publish(frameAt(1000000000LL, 1U, {0.1F, 0.2F}));
  spinFor(executor, 100ms);
  EXPECT_TRUE(received.empty());

  publisher->publish(frameAt(1002000000LL, 2U, {1.5F}));
  spinFor(executor, 100ms);
  EXPECT_TRUE(received.empty());

  publisher->publish(frameAt(1003000000LL, 3U, {0.3F, 0.4F}));
  ASSERT_TRUE(spinUntil(executor, [&received]() {
    return received.size() == 1u;
  }));
  expectSamplesNear(readSamples(received[0].data), {0.1F, 0.2F, 0.3F, 0.4F});
...
TEST_F(RclcppContractTest, SourceChangeClearsPartialBufferBeforeNewSourceWindow)
{
  auto node = std::make_shared<fa_chunk_overlap::FaChunkOverlapNode>(
    optionsWith(validParameters()));
  ...
  publisher->publish(frameAt(1000000000LL, 1U, {0.1F, 0.2F}, "mic-a"));
  spinFor(executor, 100ms);
  EXPECT_TRUE(received.empty());

  publisher->publish(frameAt(1002000000LL, 2U, {0.3F, 0.4F}, "mic-b"));
  spinFor(executor, 100ms);
  EXPECT_TRUE(received.empty());

  publisher->publish(frameAt(1004000000LL, 3U, {0.5F, 0.6F}, "mic-b"));
  ASSERT_TRUE(spinUntil(executor, [&received]() {
    return received.size() == 1u;
  }));
  EXPECT_EQ(received[0].source_id, "mic-b");
  expectSamplesNear(readSamples(received[0].data), {0.3F, 0.4F, 0.5F, 0.6F});
}

TEST_F(RclcppContractTest, DropsMismatchedStreamWithoutPublishing)
{
  auto node = std::make_shared<fa_chunk_overlap::FaChunkOverlapNode>(
    optionsWith(validParameters()));
  ...
  auto invalid = frameAt(1000000000LL, 1U, {0.1F, 0.2F, 0.3F, 0.4F});
  invalid.stream_id = "audio/wrong";
