...
void FaFrameBufferNode::loadParameters()
{
  ...
  this->declare_parameter<int>("buffering.frames_per_chunk", config_.frames_per_chunk);
  this->declare_parameter<int>("buffering.max_buffered_chunks", config_.max_buffered_chunks);
  ...
  config_.frames_per_chunk = this->get_parameter("buffering.frames_per_chunk").as_int();
  config_.max_buffered_chunks = this->get_parameter("buffering.max_buffered_chunks").as_int();
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Frame buffer config: input=%s output=%s expected=%dHz/%d/%s/%d/%s chunk_frames=%d "
