...
fa_interfaces::msg::EncodedAudioChunk makeEncodedChunk(const rclcpp::Node & node)
{
  ...
  chunk.stream_id = "fa_decode_test/input_stream";
  chunk.codec = "opus";
  chunk.container = "ogg";
...
TEST_F(RclcppFixture, PublishesAudioFrameFromEncodedInput)
{
  ...
  options.parameter_overrides({
    ...
    rclcpp::Parameter("input_stream_id", "fa_decode_test/input_stream"),
    rclcpp::Parameter("output.stream_id", "fa_decode_test/output_stream"),
  ...
  auto test_node = std::make_shared<rclcpp::Node>("fa_decode_graph_test", quietGraphNodeOptions());
  ...
  EXPECT_EQ(received->stream_id, "fa_decode_test/output_stream");
