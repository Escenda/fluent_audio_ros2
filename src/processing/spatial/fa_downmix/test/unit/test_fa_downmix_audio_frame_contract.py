...
def test_launch_requires_explicit_node_name_and_config_file() -> None:
    package_root = Path(__file__).parents[2]
    launch_text = (package_root / "launch" / "fa_downmix.launch.py").read_text(encoding="utf-8")
...
def test_downmix_preserves_metadata_and_updates_stream_channels_data_only() -> None:
    ...
    assert "out.stream_id = config_.output_stream_id;" in downmix_frame
    assert "out.stream_id = config_.output_topic;" not in downmix_frame
