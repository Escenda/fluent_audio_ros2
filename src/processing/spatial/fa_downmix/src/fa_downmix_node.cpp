...
void FaDownmixNode::loadParameters()
{
  ...
  this->declare_parameter<int>("expected.input_channels", config_.expected_input_channels);
  ...
  this->declare_parameter("mode", config_.mode);
  ...
  config_.expected_input_channels = this->get_parameter("expected.input_channels").as_int();
  ...
  config_.mode = this->get_parameter("mode").as_string();
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Downmix config: input=%s output=%s mode=%s expected=%dHz/%dch/%s/%d/%s output_channels=%d "
