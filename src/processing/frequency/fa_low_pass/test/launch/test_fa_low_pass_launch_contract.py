...
def _run_fa_low_pass_launch(config_path: Path) -> subprocess.CompletedProcess[str]:
...
def test_launch_uses_only_node_name_and_config_file_arguments() -> None:
    launch_text = (PACKAGE_ROOT / "launch" / "fa_low_pass.launch.py").read_text(
        encoding="utf-8"
    )

    assert 'DeclareLaunchArgument(\n            "node_name"' in launch_text
    assert 'DeclareLaunchArgument(\n            "config_file"' in launch_text
    assert 'FindPackageShare("fa_low_pass"), "config", "default.yaml"' in launch_text
    assert 'package="fa_low_pass"' in launch_text
    assert 'executable="fa_low_pass_node"' in launch_text
...
def test_default_launch_config_keeps_low_pass_as_frequency_node() -> None:
...
def test_launch_fails_closed_when_cutoff_is_invalid(tmp_path: Path) -> None:
    ...
    config["fa_low_pass"]["ros__parameters"]["filter"]["cutoff_hz"] = 0.0
    config_path = tmp_path / "invalid_cutoff.yaml"
    config_path.write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")

    result = _run_fa_low_pass_launch(config_path)
...
def test_launch_fails_closed_when_encoding_contract_is_wrong(tmp_path: Path) -> None:
    ...
    config["fa_low_pass"]["ros__parameters"]["expected"]["encoding"] = "PCM16LE"
    ...
    assert "process has died" in result.stdout
    assert "fa_low_pass requires expected.encoding=FLOAT32LE" in result.stdout
