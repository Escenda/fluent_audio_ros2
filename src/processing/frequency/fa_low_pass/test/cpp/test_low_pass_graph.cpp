...
double firstOrderLowPassAlpha(const double sample_rate, const double cutoff_hz)
{
  ...
  const std::vector<float> & samples)
{
  ...
  frame.stream_id = "/fa_low_pass_test/input";
  ...
  const rclcpp::Node::SharedPtr & test_node)
{
  ...
  while ((publisher->get_subscription_count() == 0U ||
          test_node->count_publishers("/fa_low_pass_test/output") == 0U) &&
  ...
  ASSERT_GT(test_node->count_publishers("/fa_low_pass_test/output"), 0U);
}
...
TEST_F(RclcppFixture, PublishesFirstOrderLowPassFloat32Frame)
{
  ...
  rclcpp::executors::SingleThreadedExecutor executor;
  executor.add_node(low_pass_node);
  ...
  executor.remove_node(low_pass_node);
  subscriber.reset();
  publisher.reset();

  ASSERT_EQ(received.size(), 2U);
  const double alpha = firstOrderLowPassAlpha(1000.0, 100.0);

  EXPECT_EQ(received[0].source_id, "test-mic");
  EXPECT_EQ(received[0].stream_id, "/fa_low_pass_test/output");
  ...
  EXPECT_NEAR(
    readFloat32Le(received[0].data, 2),
    static_cast<float>((2.0 * alpha) - (alpha * alpha)),
    1.0e-6F);

  EXPECT_EQ(received[1].epoch, 18U);
  ASSERT_EQ(received[1].data.size(), 2U * sizeof(float));
  EXPECT_NEAR(
    readFloat32Le(received[1].data, 0),
    static_cast<float>(1.0 - std::pow(1.0 - alpha, 3.0)),
    1.0e-6F);
  EXPECT_NEAR(
    readFloat32Le(received[1].data, 1),
    static_cast<float>(1.0 - std::pow(1.0 - alpha, 4.0)),
    1.0e-6F);
}
...
TEST_F(RclcppFixture, ResetsFilterStateOnForwardEpochGap)
{
  ...
  ASSERT_EQ(received[1].data.size(), 2U * sizeof(float));
  EXPECT_FLOAT_EQ(readFloat32Le(received[1].data, 0), 0.25F);
  EXPECT_NEAR(readFloat32Le(received[1].data, 1), static_cast<float>(0.25 + (0.75 * alpha)), 1.0e-6F);
}
