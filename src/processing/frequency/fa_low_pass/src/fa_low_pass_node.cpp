...
FaLowPassNode::FaLowPassNode()
: rclcpp::Node("fa_low_pass")
{
...
int main(int argc, char ** argv)
{
  ...
  try {
    auto node = std::make_shared<fa_low_pass::FaLowPassNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return EXIT_SUCCESS;
  } catch (const std::exception & e) {
    RCLCPP_FATAL(rclcpp::get_logger("fa_low_pass"), "Exception: %s", e.what());
    rclcpp::shutdown();
    return EXIT_FAILURE;
  }
}
