...
def test_launch_uses_only_node_name_and_config_file_arguments() -> None:
    launch_text = (PACKAGE_ROOT / "launch" / "fa_band_pass.launch.py").read_text(
        encoding="utf-8"
    )

    assert 'DeclareLaunchArgument(\n            "node_name"' in launch_text
    assert 'DeclareLaunchArgument(\n            "config_file"' in launch_text
    assert 'FindPackageShare("fa_band_pass"), "config", "default.yaml"' in launch_text
    assert 'package="fa_band_pass"' in launch_text
    assert 'executable="fa_band_pass_node"' in launch_text
    assert "parameters=[config_file]" in launch_text
    assert "filter.low_cut_hz" not in launch_text
    assert "filter.high_cut_hz" not in launch_text
