...
FaGainNode::FaGainNode(const rclcpp::NodeOptions & options)
: rclcpp::Node("fa_gain", options)
{
...
void FaGainNode::loadParameters()
{
  ...
  this->declare_parameter<double>("gain.linear");
  ...
  config_.linear_gain = readRequiredDouble(*this, "gain.linear");
  ...
  if (config_.expected_encoding != kEncodingFloat32) {
    throw std::runtime_error("fa_gain requires expected.encoding=FLOAT32LE");
  }
  if (config_.expected_bit_depth != 32) {
    throw std::runtime_error("fa_gain requires expected.bit_depth=32");
  }
  if (config_.expected_layout != kInterleavedLayout) {
    throw std::runtime_error("fa_gain requires expected.layout=interleaved");
  }
...
void FaGainNode::setupInterfaces()
{
  ...
  audio_sub_ = this->create_subscription<fa_interfaces::msg::AudioFrame>(
    config_.input_topic,
    qos,
    std::bind(&FaGainNode::handleFrame, this, std::placeholders::_1));
...
bool FaGainNode::validateFrame(const fa_interfaces::msg::AudioFrame & msg)
{
  ...
  fa_interfaces::msg::AudioFrame & out)
{
  ...
  if (status != backends::ProcessStatus::kOk) {
    const char * status_message = backends::processStatusMessage(status);
    RCLCPP_WARN_THROTTLE(
      this->get_logger(), *this->get_clock(), 3000,
