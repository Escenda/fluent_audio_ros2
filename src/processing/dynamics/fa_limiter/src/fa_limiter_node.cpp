...
void FaLimiterNode::loadParameters()
{
  ...
  this->declare_parameter<double>("threshold.linear", config_.threshold_linear);
  ...
  config_.threshold_linear = this->get_parameter("threshold.linear").as_double();
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Limiter config: input=%s output=%s threshold=%f expected=%dHz/%d/%s/%d qos_depth=%d reliable=%s diag=%dms",
