...
void FaLimiterNode::loadParameters()
{
  ...
  this->declare_parameter<double>("threshold.linear");
  ...
  config_.threshold_linear = readRequiredDouble(*this, "threshold.linear");
  ...
  if (config_.expected_encoding != kEncodingFloat32) {
    throw std::runtime_error("fa_limiter requires expected.encoding=FLOAT32LE");
  }
  if (config_.expected_bit_depth != 32) {
    throw std::runtime_error("fa_limiter requires expected.bit_depth=32");
  }
  if (config_.expected_layout != kInterleavedLayout) {
    throw std::runtime_error("fa_limiter requires expected.layout=interleaved");
  }
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Limiter config: input_topic=%s output_topic=%s input_stream_id=%s output_stream_id=%s threshold=%f expected=%dHz/%d/%s/%d qos_depth=%d reliable=%s diagnostics_qos_depth=%d diagnostics_reliable=%s diag=%dms",
...
void FaLimiterNode::setupInterfaces()
{
  ...
  audio_sub_ = this->create_subscription<fa_interfaces::msg::AudioFrame>(
    config_.input_topic,
    qos,
    std::bind(&FaLimiterNode::handleFrame, this, std::placeholders::_1));
  ...
  diag_timer_ = this->create_wall_timer(
    ...
    std::bind(&FaLimiterNode::publishDiagnostics, this));
}
...
bool FaLimiterNode::validateFrame(const fa_interfaces::msg::AudioFrame & msg)
{
...
void FaLimiterNode::publishDiagnostics()
{
  ...
  pushKeyValue(status, "samples_limited", std::to_string(samples_limited_.load()));
