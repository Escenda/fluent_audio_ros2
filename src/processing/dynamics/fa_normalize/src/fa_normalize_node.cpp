...
void FaNormalizeNode::loadParameters()
{
  ...
  config_.target_peak_linear = readRequiredDouble(*this, "normalize.target_peak_linear");
  config_.silence_threshold_linear =
    readRequiredDouble(*this, "normalize.silence_threshold_linear");
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Normalize config: input_topic=%s output_topic=%s input_stream_id=%s output_stream_id=%s target_peak=%f silence_threshold=%f expected=%dHz/%d/%s/%d qos_depth=%d reliable=%s diagnostics_qos_depth=%d diagnostics_reliable=%s diag=%dms",
