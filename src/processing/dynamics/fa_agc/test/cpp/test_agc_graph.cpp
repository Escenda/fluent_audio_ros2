  ...
  const uint32_t epoch)
{
  ...
  frame.data = float32LeBytes({0.25F, -0.25F, 0.25F, -0.25F});
  frame.epoch = epoch;
  return frame;
}
...
TEST_F(RclcppFixture, PublishesAgcFloat32Frame)
{
  ...
  EXPECT_EQ(received->epoch, 71U);
  ASSERT_EQ(received->data.size(), 4U * sizeof(float));
  EXPECT_FLOAT_EQ(readFloat32Le(received->data, 0), 0.5F);
