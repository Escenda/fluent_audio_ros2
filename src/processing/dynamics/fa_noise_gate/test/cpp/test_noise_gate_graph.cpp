...
TEST_F(RclcppFixture, PublishesGatedFloat32Frame)
{
  ...
  auto test_node = std::make_shared<rclcpp::Node>("fa_noise_gate_graph_test", quietGraphNodeOptions());
...
TEST_F(RclcppFixture, DropsFrameWhenStreamIdDoesNotMatchInputTopic)
{
  ...
  auto gate_node = std::make_shared<fa_noise_gate::FaNoiseGateNode>(options);
  auto test_node = std::make_shared<rclcpp::Node>("fa_noise_gate_drop_graph_test", quietGraphNodeOptions());
