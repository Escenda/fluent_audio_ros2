...
- 16kHz mono `PCM16LE/16` または `FLOAT32LE/32` の `AudioBuffer`
- `block_len=512`
- `block_shift=128`
- model 1 path
- model 2 path
...
- ONNX tensor shape が fully static / positive ではない
- input sample count が `block_shift` に揃っていない
- output sample count が input sample count と一致しない
- output sample が finite normalized range を満たさない
- inference failure
