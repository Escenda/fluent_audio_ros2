...
TEST_F(RclcppFixture, PublishesDeclickedFloat32Frame)
{
  ...
  auto test_node = std::make_shared<rclcpp::Node>("fa_declick_graph_test", quietGraphNodeOptions());
...
{
  auto declick_node = std::make_shared<fa_declick::FaDeclickNode>(validNodeOptions());
  auto test_node = std::make_shared<rclcpp::Node>("fa_declick_drop_test", quietGraphNodeOptions());
