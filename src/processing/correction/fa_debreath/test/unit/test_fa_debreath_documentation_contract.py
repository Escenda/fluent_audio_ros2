...
def test_fa_debreath_has_standard_design_documents() -> None:
    ...
    assert (PACKAGE_ROOT / "docs" / "backends" / "breath_suppression.md").is_file()


def test_fa_debreath_is_not_declared_as_ros_package_before_contract_completion() -> None:
    assert not (PACKAGE_ROOT / "package.xml").exists()
    assert not (PACKAGE_ROOT / "CMakeLists.txt").exists()


def test_fa_debreath_spec_separates_ros_topics_from_stream_identity() -> None:
