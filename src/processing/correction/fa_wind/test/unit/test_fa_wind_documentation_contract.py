...
def test_fa_wind_has_standard_design_documents() -> None:
    ...
    assert (PACKAGE_ROOT / "docs" / "backends" / "wind_noise_reduction.md").is_file()


def test_fa_wind_is_not_declared_as_ros_package_before_contract_completion() -> None:
    assert not (PACKAGE_ROOT / "package.xml").exists()
    assert not (PACKAGE_ROOT / "CMakeLists.txt").exists()


def test_fa_wind_spec_separates_ros_topics_from_stream_identity() -> None:
