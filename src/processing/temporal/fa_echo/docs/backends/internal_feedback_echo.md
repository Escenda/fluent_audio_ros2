...
`internal_feedback_echo` は外部 DSP engine を使わず、node 内部の ring delay line だけで feedback echo を処理する backend である。
...
output または next delay state が `[-1.0, 1.0]` 外になった場合、値を丸めず frame を drop する。
