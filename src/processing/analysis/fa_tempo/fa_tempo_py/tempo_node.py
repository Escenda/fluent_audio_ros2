...
from fa_interfaces.msg import AudioFrame, TempoFrame
...
class FaTempoNode(Node):
    def __init__(self) -> None:
        super().__init__("fa_tempo")
        self._load_parameters()

        self.backend = InternalOnsetAutocorrelationBackend(
            TempoConfig(
        ...
        self.tempo_pub = self.create_publisher(TempoFrame, self.output_topic, qos)
        ...
        self.get_logger().info(
            ...
            f"range={self.bpm_min:.1f}-{self.bpm_max:.1f}bpm"
        )
    ...
    def _load_parameters(self) -> None:
        ...
        self.bpm_min = self._required_double("tempo.bpm_min")
        self.bpm_max = self._required_double("tempo.bpm_max")
        self.confidence_threshold = self._required_double("tempo.confidence_threshold")
        ...
        if self.backend_name != InternalOnsetAutocorrelationBackend.name:
            raise RuntimeError("backend.name must be internal_onset_autocorrelation")
        if self.expected_sample_rate <= 0:
            raise RuntimeError("expected.sample_rate must be > 0")
        if self.expected_channels != 1:
            raise RuntimeError("fa_tempo currently requires expected.channels=1")
        if self.expected_encoding != "FLOAT32LE":
            raise RuntimeError("fa_tempo requires expected.encoding=FLOAT32LE")
        if self.expected_bit_depth != 32:
            raise RuntimeError("fa_tempo requires expected.bit_depth=32")
        if self.expected_layout != "interleaved":
            raise RuntimeError("fa_tempo requires expected.layout=interleaved")
        if self.qos_depth <= 0:
    ...
    def on_audio(self, msg: AudioFrame) -> None:
        try:
            ...
            self.get_logger().warning(f"Dropping AudioFrame before tempo publish: {exc}")
            return
        except RuntimeError as exc:
            self.get_logger().error(f"fa_tempo backend failure: {exc}")
            raise
        ...
        out.bpm_min = float(self.bpm_min)
        out.bpm_max = float(self.bpm_max)
        out.tempo_bpm = float(result.tempo_bpm)
        out.confidence = float(result.confidence)
        out.confidence_threshold = float(self.confidence_threshold)
        out.beat_period_frames = int(result.beat_period_frames)
        out.tempo_detected = bool(result.tempo_detected)
        out.method = self.method
        out.layout = "frames"
        out.frame_times_sec = result.frame_times_sec.astype(np.float32).tolist()
        out.onset_envelope = result.onset_envelope.astype(np.float32).tolist()
        out.beats = [bool(value) for value in result.beats]
        self.tempo_pub.publish(out)
