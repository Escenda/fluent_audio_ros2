...
from fa_interfaces.msg import AudioFrame, OnsetFrame
from fa_onset_py.backends.spectral_flux import (
    InternalSpectralFluxBackend,
    OnsetConfig,
)


class FaOnsetNode(Node):
    def __init__(self) -> None:
        super().__init__("fa_onset")
        self._load_parameters()

        self.backend = InternalSpectralFluxBackend(
            OnsetConfig(
        ...
        self.onset_pub = self.create_publisher(OnsetFrame, self.output_topic, qos)
        ...
        self.get_logger().info(
            ...
            f"n_fft={self.n_fft} hop={self.hop_length} threshold={self.threshold:.6f}"
        )
    ...
    def _load_parameters(self) -> None:
        ...
        self.threshold = self._required_double("detector.threshold")
        self.min_interval_sec = self._required_double("detector.min_interval_sec")
        ...
        if self.backend_name != InternalSpectralFluxBackend.name:
            raise RuntimeError("backend.name must be internal_spectral_flux")
        if self.expected_sample_rate <= 0:
            raise RuntimeError("expected.sample_rate must be > 0")
        if self.expected_channels != 1:
            raise RuntimeError("fa_onset currently requires expected.channels=1")
        if self.expected_encoding != "FLOAT32LE":
            raise RuntimeError("fa_onset requires expected.encoding=FLOAT32LE")
        if self.expected_bit_depth != 32:
            raise RuntimeError("fa_onset requires expected.bit_depth=32")
        if self.expected_layout != "interleaved":
            raise RuntimeError("fa_onset requires expected.layout=interleaved")
        if self.qos_depth <= 0:
    ...
    def on_audio(self, msg: AudioFrame) -> None:
        try:
            ...
            self.get_logger().warning(f"Dropping AudioFrame before onset publish: {exc}")
            return
        except RuntimeError as exc:
            self.get_logger().error(f"fa_onset backend failure: {exc}")
            raise
        ...
        out.min_interval_sec = float(self.min_interval_sec)
        out.method = self.method
        out.layout = "frames"
        out.frame_times_sec = result.frame_times_sec.astype(np.float32).tolist()
        out.strengths = result.strengths.astype(np.float32).tolist()
        out.detected = [bool(value) for value in result.detected]
        self.onset_pub.publish(out)
