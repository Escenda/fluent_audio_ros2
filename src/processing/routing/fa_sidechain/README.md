...
この package は program audio を購読せず、program audio の sample も変更しません。入力 sidechain frame の RMS を計算し、閾値以上なら `detector.active_gain_db`、閾値未満なら `detector.inactive_gain_db` を linear gain に変換して、mono FLOAT32LE の control frame として出力します。
...
```bash
ros2 launch fa_sidechain fa_sidechain.launch.py
```

設定例は `config/default.yaml` を参照してください。
