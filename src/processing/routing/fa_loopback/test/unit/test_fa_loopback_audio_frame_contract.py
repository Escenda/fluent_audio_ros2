...
def test_default_config_defines_explicit_loopback_routing_contract() -> None:
    ...
    assert params["input_topic"] == "audio/output/frame"
    assert params["output_topic"] == "audio/loopback/frame"
    assert params["input_topic"] != params["output_topic"]
    assert params["loopback"]["require_distinct_topics"] is True
    ...
    assert 'default_value="fa_loopback"' in launch_text
...
def test_startup_validation_fails_closed_for_invalid_config() -> None:
    ...
    assert "config_.require_distinct_topics && config_.input_topic == config_.output_topic" in load_parameters
    assert (
...
def test_loopback_copies_frame_and_only_changes_stream_id() -> None:
    ...
    assert "out.stream_id = config_.output_topic;" in publish_loopback
...
def test_diagnostics_report_config_and_counters() -> None:
    ...
    assert "loopback.require_distinct_topics" in diagnostics
