...
- **ASR**: 標準経路では `fa_asr` が `parakeet_multilingual_buffered` backend を内部に保持し、multilingual Parakeet 1.1B `.nemo` を 16 kHz mono `FLOAT32LE` rolling buffer / chunked re-decode policy で呼び、未確定 partial と final `voice/asr/result` を Publish する。Whisper、OpenAI、NIM、Riva、gRPC、stdin/stdout JSONL worker へ fallback しない。
