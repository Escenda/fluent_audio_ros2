...
- `src/io/sinks/`: 出力 sink adapter / 収録 / 配信（`fa_out`, `fa_record`, `fa_stream`）
...
- `src/processing/analysis/`: 解析/特徴量/音声AI（`fa_vad`, `fa_kws`, `fa_asr`, `fa_turn_detector`, `fa_log_mel`, `fa_audio_embedding`, `fa_sed`, `fa_speaker`）
...
- `fa_vad`: `src/processing/analysis/fa_vad/config/default_vad.yaml`（root: `fa_vad_node`）
...
- `fa_kws`: `src/processing/analysis/fa_kws/config/default.yaml`（root: `fa_kws`）
- `fa_asr`: `src/processing/analysis/fa_asr/config/default.yaml`（root: `fa_asr`）
- `fa_turn_detector`: `src/processing/analysis/fa_turn_detector/config/default.yaml`（root: `fa_turn_detector`）
- `fa_record`: `src/io/sinks/fa_record/config/default.yaml`（root: `fa_record`）
- `fa_stream`: `src/io/sinks/fa_stream/config/default.yaml`（root: `fa_radio_streamer`）
...
6. `src/apps/fa_dialogue`: `voice/wake_word` / `voice/asr/result` / `voice/turn_end` を合流し、LLM/TTS/ロボット操作へ接続する会話アプリ層を実装
