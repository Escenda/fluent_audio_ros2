...
| `fa_audio_window` | PCM16LE/WAV evidence rolling window。numeric range の export / archive service、決定的 `AudioClipRef` / path、`metadata_uri` / `content_sha256` / `metadata_sha256`、archive metadata sidecar、filesystem archive store contract、同一 request の idempotent 応答、既存 deterministic target conflict の fail-closed を Docker/VLAbor ROS2 package test で確認済み。`colcon build --packages-select fa_interfaces fa_audio_window fa_audio_mcp fluent_audio_system` passed、`colcon test --packages-select fa_interfaces fa_audio_window fa_audio_mcp --event-handlers console_direct+` passed、service contract test は 11 tests、package test summary は 3 test binaries all passed | 実 R2/S3 client upload、World Station evidence linkage、親 Agent Runtime MCP client、親 VLAbor profile integration、実 SO101 device / model provisioning は未検証 |
