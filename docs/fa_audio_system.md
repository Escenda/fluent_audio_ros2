...
| `fa_asr` | 標準 ASR path は `parakeet_multilingual_buffered`。multilingual Parakeet 1.1B `.nemo` を backend/session 内部 runner として保持し、`FLOAT32LE` 16 kHz mono rolling buffer を chunk 境界で再 decode して未確定 partial hypothesis を返し、`finish()` で final transcript を commit する。`TranscribeAudio` は ASR-ready `FLOAT32LE` rolling timeline だけを使い、`window_not_found` / `range_outside_window` / `range_not_continuous` を区別する。 | 実 local `.nemo` model による live microphone full graph、generic accuracy、親 VLAbor profile integration、実 SO101 device provisioning は未検証 |
...
`src/system/fluent_audio_system/test/integration/test_combined_owner_mcp_launch_smoke.py::test_so101_profile_pair_runs_real_asr_model_when_enabled` は、同じ実 SO101 profile pair launch path を使い、ASR だけを実 model / audio fixture に差し替える opt-in smoke です。default では `FLUENT_AUDIO_REAL_ASR_SMOKE=1` がない限り skip します。enabled 時は `FLUENT_AUDIO_ASR_MODEL_PATH` が読み取り可能 file、`FLUENT_AUDIO_REAL_ASR_AUDIO_F32_PATH` が読み取り可能かつ非空の raw `FLOAT32LE` mono 16kHz fixture であることを fail closed で要求します。fixture は byte length が 4 の倍数で、有限かつ `[-1.0, 1.0]` の normalized sample だけを含む必要があります。`FLUENT_AUDIO_REAL_ASR_EXPECTED_TEXT` が設定されていれば transcript にその文字列を含むこと、未設定なら transcript が非空であることを確認します。`FLUENT_AUDIO_REAL_ASR_SAMPLE_RATE` を設定する場合は `16000` だけを許可し、この smoke は resampling を隠しません。

この real ASR opt-in smoke は、fake VAD / KWS / Turn Detector fixture を引き続き使い、実 SO101 device provisioning は行いません。検証対象は ROS service `TranscribeAudio` の success、segment time range、`model_ref.backend_name == parakeet_multilingual_buffered`、`model_ref.backend_kind == asr`、`model_ref.model_path` が指定 model path と一致することです。実 model / audio fixture による成功経路は、この環境では必要 env が提供されていないため未検証です。
