...
| `fa_asr` | `local_command`、`whisper.cpp`、`parakeet_worker`、`openai_realtime`、`openai_transcriptions` backend 枠。`TranscribeAudio` は ASR-ready `FLOAT32LE` rolling timeline だけを使い、`window_not_found` / `range_outside_window` / `range_not_continuous` を区別する。host unit test は `test_transcribe_audio_timeline.py` で 28 passed、Docker/VLAbor package test は `fa_asr` 69 passed。SO101 profile pair 上で実 ASR worker / model / audio fixture を使う opt-in smoke は追加済み | 実 worker command / model / audio fixture を指定した opt-in smoke の成功経路は、この環境では env 未指定のため未検証。実 credential、親 VLAbor profile integration、実 SO101 device provisioning も未検証 |
| `fa_audio_window` | PCM16LE/WAV evidence rolling window。numeric range の export / archive service、決定的 `AudioClipRef` / path、`metadata_uri` / `content_sha256` / `metadata_sha256`、archive metadata sidecar、filesystem archive store contract、同一 request の idempotent 応答、既存 deterministic target conflict の fail-closed、既存 durable filesystem target 要求、startup preflight、publish 時 unavailable failure を Docker/VLAbor ROS2 package test で確認済み。`colcon build --packages-select fa_interfaces fa_audio_window fa_audio_mcp fluent_audio_system` passed、`colcon test --packages-select fa_interfaces fa_audio_window fa_audio_mcp --event-handlers console_direct+` passed、service contract test は 13 tests、package test summary は 3 test binaries all passed | 実 R2/S3 client upload、World Station evidence linkage、親 Agent Runtime MCP client、親 VLAbor profile integration、実 SO101 device / model provisioning は未検証 |
...
| `fa_audio_mcp` | `archive_audio_window` / `transcribe_audio` MCP adapter、numeric range / ROS node clock relative range / `FLUENT_AUDIO_TIME_MARKERS` 由来 marker endpoint range の adapter 境界解決、`AudioClipRef.metadata_uri` / `content_sha256` / `metadata_sha256` formatter、SO101 agent audio tools profile、in-process `FastMCP` から実 `fa_asr_node` / `fa_audio_window_node` を呼ぶ Docker runtime smoke、installed server に streamable-http MCP transport で接続して実 owner node へ tool call する smoke、`so101_voice_frontend.yaml,so101_agent_audio_tools.yaml` の child-side FluentAudio config composition / required package order 検証、`owner.yaml,adapter.yaml` multi-config list を `ros2 launch fluent_audio_system run.py` に渡して `fa_asr` / `fa_audio_window` owner services と `fa_audio_mcp_server` adapter executable を同時起動する Docker ROS runtime smoke。Docker/VLAbor ROS2 package test は `fa_audio_mcp` 101 passed | 実 Agent Runtime / MCP client、親 Agent Runtime / World Station 側の自然言語 resolver / turn marker / artifact resolver、親 VLAbor profile integration、実 R2/S3 client upload / World Station evidence linkage、実 SO101 device / model provisioning の検証完了は意味しない |
...
この config composition は child repo の FluentAudio package-local 機能です。`so101_voice_frontend.yaml,so101_agent_audio_tools.yaml` を 1 つの FluentAudio system config として読むことは確認できますが、それだけで親 VLAbor profile integration、Agent Runtime / MCP client integration、実 R2/S3 client upload、World Station evidence linkage、実 SO101 device / model provisioning が検証済みになったわけではありません。multi-config の代表 runtime launch smoke は後述の combined launch smoke で確認しています。
...
`src/system/fluent_audio_system/test/integration/test_combined_owner_mcp_launch_smoke.py` は、`config:=owner.yaml,adapter.yaml` 形式の multi-config list を `ros2 launch fluent_audio_system run.py` に渡す generated owner/adapter smoke と、実 profile pair `${share:fluent_audio_system}/config/profiles/so101_voice_frontend.yaml,${share:fluent_audio_system}/config/profiles/so101_agent_audio_tools.yaml` を起動する SO101 profile pair runtime smoke を含みます。fake worker / generated params または test fixture の fake worker / fake model artifact を使いながら、実 `fa_asr` / `fa_asr_node`、`fa_audio_window` / `fa_audio_window_node` owner services と `fa_audio_mcp` / `fa_audio_mcp_server` adapter を同じ launch graph に載せます。起動済み MCP adapter ROS node、real `TranscribeAudio` / `ExportAudioWindow` / `ArchiveAudioWindow` service、deterministic `FLOAT32LE` ASR frame、deterministic `PCM16LE` archive frame、service response の transcript / model ref / window ref、WAV bytes、archive metadata を runtime behavior として検証します。

`src/system/fluent_audio_system/test/integration/test_combined_owner_mcp_launch_smoke.py::test_so101_profile_pair_runs_real_asr_worker_and_model_when_enabled` は、同じ実 SO101 profile pair launch path を使い、ASR だけを実 worker / model / audio fixture に差し替える opt-in smoke です。default では `FLUENT_AUDIO_REAL_ASR_SMOKE=1` がない限り skip します。enabled 時は `FLUENT_AUDIO_ASR_WORKER` が実行可能 file、`FLUENT_AUDIO_ASR_MODEL_PATH` が読み取り可能 file、`FLUENT_AUDIO_REAL_ASR_AUDIO_F32_PATH` が読み取り可能かつ非空の raw `FLOAT32LE` mono 16kHz fixture であることを fail closed で要求します。fixture は byte length が 4 の倍数で、有限かつ `[-1.0, 1.0]` の normalized sample だけを含む必要があります。`FLUENT_AUDIO_REAL_ASR_EXPECTED_TEXT` が設定されていれば transcript にその文字列を含むこと、未設定なら transcript が非空であることを確認します。`FLUENT_AUDIO_REAL_ASR_SAMPLE_RATE` を設定する場合は `16000` だけを許可し、この smoke は resampling を隠しません。

この real ASR opt-in smoke は、fake VAD / KWS / Turn Detector fixture を引き続き使い、実 SO101 device provisioning は行いません。検証対象は ROS service `TranscribeAudio` の success、segment time range、`model_ref.backend_name == whisper.cpp`、`model_ref.backend_kind == asr`、`model_ref.model_path` が指定 model path と一致することです。実 worker / model / audio fixture による成功経路は、この環境では必要 env が提供されていないため未検証です。
...
今回の real ASR opt-in smoke 追加後の Docker/VLAbor 代表 system validation では、SO101 profile package set を含めた isolated build が通り、`colcon test --packages-select fluent_audio_system` は `197 passed, 1 skipped` として確認済みです。skip は real ASR smoke の default opt-in skip であり、実 ASR env を与えた成功経路の確認ではありません。`FLUENT_AUDIO_TIME_MARKERS` 由来の marker endpoint resolver は、config validation、marker endpoint / offset 解決、未設定 / 不正 marker の fail-closed、request builder 境界での numeric `time_range_spec` 化、MCP tool 境界で service 呼び出し前に marker spec を解決する経路を package-local tests で確認しています。installed streamable-http relative-time smoke は `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python3 -m pytest src/apps/agent_tools/fa_audio_mcp/test/test_streamable_http_transport_smoke.py -q` で `2 passed in 9.35s` として確認済みです。

この combined launch smoke と streamable transport smoke で確認した owner 境界は、`transcribe_audio` の owner が `fa_asr`、`archive_audio_window` / `export_audio_window` の owner が `fa_audio_window` であることです。combined launch smoke は child-side `fluent_audio_system` multi-config launch composition の検証であり、streamable transport smoke は child repo の installed server transport / tool-call smoke です。親 VLAbor profile integration、親 Agent Runtime / MCP client 統合、親 Agent Runtime / World Station 側の自然言語 resolver / turn marker / artifact resolver、実 R2/S3 client upload、World Station evidence linkage、実 SO101 device / model provisioning は検証していません。
