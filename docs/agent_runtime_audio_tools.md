...
実 SO101 FluentAudio profile pair runtime smoke として、`src/system/fluent_audio_system/test/integration/test_combined_owner_mcp_launch_smoke.py::test_so101_profile_pair_runs_owner_services_and_mcp_tools` は targeted 実行で `1 passed in 9.13s` として確認済みである。この smoke は `${share:fluent_audio_system}/config/profiles/so101_voice_frontend.yaml,${share:fluent_audio_system}/config/profiles/so101_agent_audio_tools.yaml` を `fluent_audio_system/run.py` 経由で起動し、test fixture の fake worker / fake model artifact と deterministic high-pass input frame を使って、`fa_audio_mcp -> fa_asr/fa_audio_window` の runtime service/tool 経路を確認する。同じ test file 全体は Docker ROS2 環境で `fluent_audio_system` rebuild 後に `2 passed in 10.65s` として確認済みである。

現在の `fluent_audio_system` `config` / `--config` はカンマ区切りの明示 config path list を受け付け、左から右の順序を保持して child-side FluentAudio config を compose できる。composition は fail closed で、空 segment、missing config、invalid YAML、`system.default_start_delay` / `system.inter_group_delay` 不一致、group id 重複、enabled node id 重複を成功扱いしない。上記 profile pair smoke は実 package-owned FluentAudio profile pair の child repo runtime smoke である。ただし、親 VLAbor profile integration、親 Agent Runtime / MCP client integration、durable storage、World Station evidence linkage、実 SO101 device / production model provisioning は検証済みではない。
...
この transport smoke は child repo の installed server transport / relative-time adapter / tool behavior の検証である。自然言語 time-range resolver、marker resolver、親 repo の Agent Runtime / MCP client 統合、VLAbor agent 統合、World Station evidence 連携、durable storage、実 SO101 device / production model provisioning は pending である。
...
これらは runtime behavior の検証である。`now-10s..now` の support は child-side `fa_audio_mcp` adapter 境界で確認済みであり、`fa_asr` / `fa_audio_window` owner は numeric / resolved range を受け取る。実 SO101 FluentAudio profile pair smoke も child repo 内で確認済みである。ただし、親 Agent Runtime / MCP client、自然言語 resolver、marker resolver、World Station durable storage、実 SO101 device / production model provisioning はまだ検証していない。

Docker cleanup fix 後の `fa_audio_mcp` package 検証では、`colcon test --packages-select fa_audio_mcp --event-handlers console_direct+` が 79 items を収集し、`79 passed in 6.76s`、package は `7.51s` で完了している。今回の installed streamable-http relative-time smoke は `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python3 -m pytest src/apps/agent_tools/fa_audio_mcp/test/test_streamable_http_transport_smoke.py -q` で `2 passed in 9.35s` として確認済みである。
...
以下は pending work を含む作業パッケージである。child repo の MCP adapter、combined launch smoke、実 SO101 FluentAudio profile pair smoke、installed streamable-http transport smoke に実装済みの範囲は明記し、親 repo Agent Runtime、親 VLAbor 起動全体、実 SO101 device / production model provisioning、storage 連携は完了扱いしない。
