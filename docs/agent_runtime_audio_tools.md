...
`fa_audio_window` 側には、FluentAudio 内の最小 durable media identity slice として `AudioClipRef` の hash fields と `archive.store.backend` がある。既定の `local_file` backend は既存 local archive / export behavior を保つ。`filesystem` backend は archive 成功時に WAV / metadata JSON を configured store directory へ deterministic / no-clobber publish し、`file://` ではない URI prefix から `uri` / `metadata_uri` を返す contract backend である。これは実 R2/S3 client upload の実装ではない。
...
`local_file` backend は既存の local archive behavior を保つ。`filesystem` backend は local output に加えて `archive.store.directory` へ WAV と metadata JSON を deterministic / no-clobber publish し、`archive.store.uri_prefix` / `archive.store.metadata_uri_prefix` から `AudioClipRef.uri` / `metadata_uri` を構成する。`filesystem` backend の URI prefix は `file://` を禁止し、必須設定が欠ける場合は起動時 fail closed とする。
