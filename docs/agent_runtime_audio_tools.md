...
実 owner node smoke として、`test/test_real_owner_graph_smoke.py::test_mcp_tools_call_real_asr_and_audio_window_owner_nodes` は Docker 内で `1 passed` として確認済みである。この smoke は `ros2 run` で実 `fa_asr_node` / `fa_audio_window_node` を起動し、in-process `FastMCP` tool から real `RosAudioTimelineClient` 経由で両 service を呼ぶ。一方で、自然言語 time-range resolver、MCP HTTP / streamable transport 経由の tool call、親 repo の Agent Runtime / VLAbor agent 統合、World Station evidence 連携、durable storage は pending である。
...
この combined launch smoke は、installed `fa_audio_mcp_server` executable が `fluent_audio_system` launch 経由で ROS node/process として起動することを確認する。一方で、MCP HTTP / streamable transport 経由の tool call、親 repo の Agent Runtime / MCP client 統合、World Station durable storage、実 SO101 model / device provisioning は検証しない。
...
どちらの smoke も runtime behavior の検証である。ただし、installed MCP HTTP / streamable transport 経由の tool call、親 Agent Runtime / MCP client、World Station durable storage、実 SO101 model / device provisioning はまだ検証していない。

## 8. 次フェーズの作業パッケージ

以下は pending work を含む作業パッケージである。child repo の MCP adapter と combined launch smoke に実装済みの範囲は明記し、親 repo Agent Runtime、SO101 profile merge、MCP transport tool call、storage 連携は完了扱いしない。
...
現時点で確認済みなのは profile expansion と required package list、in-process `FastMCP` adapter が実 ROS graph 上の `transcribe_audio` / `archive_audio_window` service と接続できること、および generated single system YAML を `fluent_audio_system/run.py` で起動して `fa_asr` / `fa_audio_window` owner nodes と `fa_audio_mcp_server` adapter executable を同時に launch できることである。

combined launch smoke は profile merge support ではない。`so101_voice_frontend.yaml` と `so101_agent_audio_tools.yaml` を親側 include でどう組み合わせるか、Agent Runtime / MCP client から tool をどう呼ぶかは別検証である。

### 8.5 残る integration smoke tests

次は pending work として残る。

- installed `fa_audio_mcp_server` の MCP HTTP / streamable transport 経由 tool-call smoke
...
実 owner node smoke は、numeric range に解決済みの request で `transcribe_audio` / `archive_audio_window` が実 service owner に到達することを検証済みである。combined launch smoke は、generated single system YAML と `fluent_audio_system/run.py` 経由で owner nodes と adapter executable が同じ launch graph に載り、実 owner service behavior が成立することを検証済みである。ただし、上記の transport、Agent Runtime、durable storage 経路は検証していない。
