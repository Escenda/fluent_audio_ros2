...
実 owner node smoke として、`test/test_real_owner_graph_smoke.py::test_mcp_tools_call_real_asr_and_audio_window_owner_nodes` は Docker 内で `1 passed` として確認済みである。この smoke は `ros2 run` で実 `fa_asr_node` / `fa_audio_window_node` を起動し、in-process `FastMCP` tool から real `RosAudioTimelineClient` 経由で両 service を呼ぶ。一方で、自然言語 time-range resolver、installed `fa_audio_mcp_server` executable / MCP transport、profile-level combined `fluent_audio_system` launch、親 repo の Agent Runtime / VLAbor agent 統合、World Station evidence 連携、durable storage は pending である。
...
現在確認済みの runtime smoke は `src/apps/agent_tools/fa_audio_mcp/test/test_real_owner_graph_smoke.py::test_mcp_tools_call_real_asr_and_audio_window_owner_nodes` である。この smoke は実 `fa_asr_node` / `fa_audio_window_node`、deterministic ASR / archive frame、in-process `FastMCP`、real `RosAudioTimelineClient` を使い、成功 result と owner-node unsupported scope error を確認する。これは profile-level combined launch smoke や installed MCP server transport smoke ではない。

## 8. 次フェーズの作業パッケージ

以下は pending work を含む作業パッケージである。child repo の MCP adapter に実装済みの範囲は明記し、親 repo Agent Runtime、profile-level launch、transport、storage 連携は完了扱いしない。
...
現時点で確認済みなのは profile expansion と required package list、および in-process `FastMCP` adapter が実 ROS graph 上の `transcribe_audio` / `archive_audio_window` service と接続できることである。`fluent_audio_system` profile で owner nodes と adapter を同時に起動する combined launch smoke は未検証である。

### 8.5 残る integration smoke tests

次は pending work として残る。

- installed `fa_audio_mcp_server` executable を実際に起動する MCP transport smoke
- `fluent_audio_system` profile で `fa_asr` / `fa_audio_window` owner nodes と `fa_audio_mcp` adapter を組み合わせる combined launch smoke
...
実 owner node smoke は、numeric range に解決済みの request で `transcribe_audio` / `archive_audio_window` が実 service owner に到達することを検証済みである。ただし、上記の profile、transport、Agent Runtime、durable storage 経路は検証していない。
