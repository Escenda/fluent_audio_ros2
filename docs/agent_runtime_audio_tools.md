...
この transport smoke は child repo の installed server transport / relative-time adapter / tool behavior の検証である。自然言語 time-range resolver、marker resolver、親 repo の Agent Runtime / MCP client 統合、VLAbor agent 統合、World Station evidence 連携、durable storage、real SO101 runtime / device provisioning は pending である。
...
これらは runtime behavior の検証である。`now-10s..now` の support は child-side `fa_audio_mcp` adapter 境界で確認済みであり、`fa_asr` / `fa_audio_window` owner は numeric / resolved range を受け取る。transport smoke は child repo の installed server transport / relative-time tool-call smoke であり、親 Agent Runtime / MCP client、自然言語 resolver、marker resolver、World Station durable storage、実 SO101 model / device provisioning はまだ検証していない。
