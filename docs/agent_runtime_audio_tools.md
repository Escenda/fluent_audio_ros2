...
この transport smoke は child repo の installed server transport / relative-time adapter / tool behavior の検証である。自然言語 time-range resolver、marker resolver、親 repo の Agent Runtime / MCP client 統合、VLAbor agent 統合、World Station evidence 連携、実 R2/S3 client upload、実 SO101 device / production model provisioning は pending である。
...
これらは runtime behavior の検証である。`now-10s..now` の support は child-side `fa_audio_mcp` adapter 境界で確認済みであり、`fa_asr` / `fa_audio_window` owner は numeric / resolved range を受け取る。実 SO101 FluentAudio profile pair smoke も child repo 内で確認済みである。ただし、親 Agent Runtime / MCP client、自然言語 resolver、marker resolver、World Station evidence artifact、実 R2/S3 client upload、実 SO101 device / production model provisioning はまだ検証していない。

今回の `fa_audio_mcp` package 検証では、Docker/VLAbor ROS2 環境の `colcon test --packages-select fa_interfaces fa_audio_window fa_audio_mcp --event-handlers console_direct+` で `fa_audio_mcp` が `80 passed` として確認済みである。host `test_responses.py` は `6 passed` で、MCP formatter が `AudioClipRef` の新 fields を JSON に出すことも検証対象に含む。installed streamable-http relative-time smoke は `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python3 -m pytest src/apps/agent_tools/fa_audio_mcp/test/test_streamable_http_transport_smoke.py -q` で `2 passed in 9.35s` として確認済みである。
...
child repo の runtime smoke で検証済みなのは numeric `<start_unix_ns>..<end_unix_ns>` range を service request 境界で扱う経路と、MCP adapter 境界で `now-10s..now` を ROS node clock により numeric range へ解決してから owner services へ渡す経路である。

pending として残るのは、Agent Runtime 側の自然言語 / marker / artifact range resolver と、親 Agent Runtime / MCP client 経由の呼び出しである。
